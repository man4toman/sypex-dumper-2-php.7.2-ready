...
class Sypex_Dumper {
	...
	function connect($host = null, $port = null, $user = null, $pass = null){
		...
		if($this->mysqli = mysqli_connect($this->CFG['my_host'] . ($this->CFG['my_host'][0] != ':' ? ":{$this->CFG['my_port']}" : ''),  $this->CFG['my_user'], $this->CFG['my_pass'], '')) {
			...
			*/
			$this->mysqli->query('SET NAMES utf8') or sxd_my_error();
	...
	function deleteDB($name){
		$r = $this->mysqli->query('DROP DATABASE `' . sxd_esc($name, false) . '`') or sxd_my_error();
		if($r){
        	echo "sxd.clearOpt('db');sxd.addOpt(" . sxd_php2json(array('db' => $this->getDBList())) . ");sxd.combos.services_db.select(0,'-');";
		}
        else
        	echo "alert(" . sxd_esc($this->mysqli->error) . ");";
	}
	function cfg2js($cfg){
		foreach($cfg AS $k => $v){
	...
	function addDb($req){
        $r = $this->mysqli->query('CREATE DATABASE `' . sxd_esc($req['name'], false) . '`' . (V_MYSQL > 40100 ? "CHARACTER SET {$req['charset']} COLLATE {$req['collate']}" : ''));
        if($r)
	...
	function runServices($job) {
		...
		if(isset($serv[$job['type']])) {
			$this->mysqli->select_db($job['db']);
			$filter = $object = array();
			$this->createFilters($job['obj'], $filter, $object);
			$r = $this->mysqli->query('SHOW TABLE STATUS') or sxd_my_error();
			if (!$r) return;
			$tables = array();
			...
			$r = $this->mysqli->query($sql) or sxd_my_error();
			if (!$r) return;
			$res = array();
	...
	function runRestoreJob($continue = false){
		...
		$this->mysqli->select_db($this->JOB['db']);
		...
		if($this->JOB['strategy'] > 0){
			$r = $this->mysqli->query("SHOW TABLES") or sxd_my_error();
			while($item = $r->fetch_row()){
		...
		//$enable_index = array();
		if(V_MYSQL > 40014) {
			$this->mysqli->query("SET UNIQUE_CHECKS=0");
			$this->mysqli->query("SET FOREIGN_KEY_CHECKS=0");
			if(V_MYSQL > 40101) $this->mysqli->query("SET SQL_MODE='NO_AUTO_VALUE_ON_ZERO'");
			if(V_MYSQL > 40111) $this->mysqli->query("SET SQL_NOTES=0");
		}
		$log_sql = false;
		$fields = '';
		$time_old = time();
			...
			switch($q[0]){
				...
				case 'I':
					if (preg_match('/^INSERT( INTO `(.+?)`) VALUES/', $q, $m)) {
						...
						$this->mysqli->query("ALTER TABLE `{$tab}` DISABLE KEYS") or sxd_my_error();
...
//						$this->setNames($this->JOB['correct'] == 1 && !empty($this->JOB['charset']) ? $this->JOB['charset'] : empty($m[3]) ? '' : $m[3]);
						...
						if($m[1] == 'TC') {
							...
							if($tc && ($this->JOB['strategy'] == 0 || isset($tab_exists[$m[2]]))) {
								$this->mysqli->query("{$tc} `{$m[2]}`") or sxd_my_error();
							}
						}
						elseif($m[1] == 'TD'){
							$ei = 1;
							...
							$this->rtl[7] = 0;
							$this->rtl[8] = 0;
							$this->mysqli->query("DROP {$types[$m[1]]} IF EXISTS `{$m[2]}`") or sxd_my_error();
		...
		foreach($this->JOB['todo']['TA'] AS $tab){
			...
			$this->mysqli->query("ALTER TABLE `{$tab[1]}` ENABLE KEYS") or sxd_my_error();
			$this->rtl[1] = time();
			$this->rtl[5] = $tab[1];
			fseek($this->fh_rtl, 0);
			fwrite($this->fh_rtl, implode("\t", $this->rtl));
	...
	function addBackupJob($job) {
		...
		foreach($queries AS $query){
			...
			if($t == 'TA' && (!empty($object['TC']) || !empty($filter['TC']))) {}
			elseif(empty($object[$t]) && empty($filter[$t])) continue;
			$r = $this->mysqli->query('SHOW ' . $query[0]) or sxd_my_error();
			if (!$r) continue;
			$todo[$t] = array();
			$header[$t] = array();
		...
		if (V_MYSQL > 50014 && (!empty($object['VI']) || !empty($filter['VI']))) {
			...
			$r = $this->mysqli->query("SELECT table_name, view_definition /*!50121 , collation_connection */ FROM INFORMATION_SCHEMA.VIEWS WHERE TABLE_SCHEMA = '{$this->JOB['db']}'") or sxd_my_error();
	...
	function runBackupJob($continue = false){
		...
		$this->mysqli->query("SET SQL_QUOTE_SHOW_CREATE = 1");
		$types = array('VI' => 'View', 'PR' => 'Procedure', 'FU' => 'Function', 'TR' => 'Trigger', 'EV' => 'Event');
		$fcache = '';
		$writes = 0;
		
		if(V_MYSQL > 40101) $this->mysqli->query("SET SESSION character_set_results = '" . ($this->JOB['charset'] ? $this->JOB['charset'] : 'binary') ."'") or sxd_my_error();
		$time_old = time();
		$exit_time = $time_old + $this->CFG['time_web'] - 1;
				...
				switch($n[0]){
					...
					case 'TA':
						...
						if ($n[0] == 'TC' || $this->rtl[7] == 0){
							...
							$r = $this->mysqli->query("SHOW CREATE TABLE `{$n[1]}`") or sxd_my_error();
							$item = $r->fetch_assoc();
						    $fcache .= "#\tTC`{$n[1]}`{$n[2]}\t;\n{$item['Create Table']}\t;\n";
						...
						$notNum = array();
						$r = $this->mysqli->query("SHOW COLUMNS FROM `{$n[1]}`") or sxd_my_error();
			            $fields = 0;
			            ...
			            $z = 0;
			            // Достаем данные
			            $r = $this->mysqli->query("SELECT {$no_cache}* FROM `{$n[1]}`{$from}", MYSQLI_USE_RESULT);
			            while($row = $r->fetch_row()) {
							...
							for($k = 0; $k < $fields; $k++){
								...
								elseif($notNum[$k]) {$row[$k] =  '\'' . $this->mysqli->real_escape_string($row[$k]) . '\'';} // TODO: Потестить скорость эскэйпинга строк
							}
							$fcache .= '(' . implode(',', $row) . "),\n";
							$this->rtl[7]++;  
							$this->rtl[10]++;
					...
					default:
						if(V_MYSQL < 50121 && $n[0] == 'TR'){
							...
							$r = $this->mysqli->query("SELECT * FROM `INFORMATION_SCHEMA`.`TRIGGERS` WHERE `TRIGGER_SCHEMA` = '{$this->JOB['db']}' AND `TRIGGER_NAME` = '{$n[1]}'") or sxd_my_error();
							$item = $r->fetch_assoc();
							...
							$r = $this->mysqli->query("SHOW CREATE {$types[$n[0]]} `{$n[1]}`") or sxd_my_error();
							$item = $r->fetch_assoc();
	...
	function setNames($collation){
		...
		if($this->rtl[6] != $collation) {
			$this->mysqli->query('SET NAMES \'' . preg_replace('/^(\w+?)_/', '\\1\' COLLATE \'\\1_', $collation) . '\'') or sxd_my_error();
	...
	function getDBList(){
		...
		if ($this->CFG['my_db']) {
			...
			$result = $this->mysqli->query("SHOW DATABASES") or sxd_my_error();
		...
		if(V_MYSQL < 50000){
			foreach($items AS $item){
    			$tables = $this->mysqli->query("SHOW TABLES FROM `{$item}`") or sxd_my_error();
    			if ($tables) {
			...
			$result = $this->mysqli->query("SELECT `table_schema`, COUNT(*) FROM `information_schema`.`tables` {$where} GROUP BY `table_schema`") or sxd_my_error();
			while($item = $result->fetch_row()){
    			if($item[0] == 'information_schema' || $item[0] == 'mysql' || $item[0] == 'performance_schema') continue;
	...
	function getCharsetList(){
		...
		if(V_MYSQL > 40101) {
    		...
    		$r = $this->mysqli->query("SHOW CHARACTER SET") or sxd_my_error(); 
    		if ($r) {
    			while($item = $r->fetch_assoc()){
	...
	function getCollationList(){
		...
		if(V_MYSQL > 40101) {
    		...
    		$r = $this->mysqli->query("SHOW COLLATION") or sxd_my_error(); 
    		if ($r) {
    			while($item = $r->fetch_assoc()){
	...
	function getObjects($tree, $db_name){
		...
		// Достаем таблицы
		$r = $this->mysqli->query('SHOW TABLE STATUS');
		...
		if($r){
			...
			if (V_MYSQL > 50014 && $tree != 'services') {
				...
				for($i = 0, $l = count($shows); $i < $l; $i++){
					$r = $this->mysqli->query('SHOW ' . $shows[$i]);
					if($r && $r->num_rows > 0) {
